---

title: Budgeteen
type: kanban
status: active
tags:
  - obsidian
  - kanban
  - mon-budget
kanban-plugin: board

---

## To-do

- [ ] 3A Renforcer les sélecteurs E2E
- [ ] 3B Vérifier la persistance après rechargement
- [ ] 3C Évaluer le support multi-navigateurs
- [ ] 3D Ajouter un smoke test du site déployé
- [ ] 3E Tester le swipe sur la page budget
- [ ] Export OFX
- [ ] Import OFX
- [ ] UI dashboard pour OFX
- [ ] Fullscreen pour PieChart et LineChart
- [ ] Tests E2E du fullscreen
- [ ] Rendre le BarChart du dashboard cliquable vers `/budget`
- [ ] Ajouter un hint d'animation pour le swipe budget


## In Progress


## Done

**Complete**
- [ ] Stabiliser complètement les tests E2E export/import [[tasks/stabiliser-tests-e2e-export-import/task|note]]
- [ ] Réorganisation documentaire Obsidian: consolidation vers `research.md` et `plan.md`
- [ ] Pivot architecture vers une PWA locale offline-first
- [ ] Mettre en place la navbar responsive mobile
- [ ] Remplacer la navbar mobile par une bottom navigation
- [ ] Corriger le responsive des cartes mobile
- [ ] Corriger le `BASE_PATH` pour GitHub Pages
- [ ] Ajouter le fallback `404.html` et `.nojekyll` pour GitHub Pages
- [ ] Mettre en place l'export/import JSON
- [ ] Migrer `transactions/+page.svelte` vers Svelte 5
- [ ] Migrer le dashboard `src/routes/+page.svelte` vers Svelte 5
- [ ] Extraire les couleurs hardcodées des charts SVG
- [ ] Normaliser les overlays et fonds RGBA




%% kanban:settings
```
{"kanban-plugin":"board","list-collapse":[false,false,false]}
```
%%
